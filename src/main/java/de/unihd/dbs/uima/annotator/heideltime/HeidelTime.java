...
public class HeidelTime extends JCasAnnotator_ImplBase {
    ...
    public static final Pattern PATTERN_UPPERCASE = Pattern.compile("%UPPERCASE%\\((.*?)\\)");
    public static final Pattern PATTERN_LOWERCASE = Pattern.compile("%LOWERCASE%\\((.*?)\\)");
    public static final Pattern PATTERN_SUBSTRING = Pattern.compile("%SUBSTRING%\\((.*?),([0-9]+),([0-9]+)\\)");
    public static final Pattern PATTERN_GROUP = Pattern.compile("group\\(([0-9]+)\\)");
    public static final Pattern PATTERN_NORM = Pattern.compile("%([A-Za-z0-9]+?)\\(group\\(([0-9]+)\\)\\)");
    public static final Pattern PATTERN_CONSTRAINT = Pattern.compile("group\\(([0-9]+)\\):(.*?):");
    public static final Pattern PATTERN_TO_CALCULATE = Pattern.compile("^(UNDEF-(this|REFUNIT|REF)-(.*?)-(MINUS|PLUS)-([0-9]+)).*");
    public static final Pattern PATTERN_MONTH_NAMES = Pattern.compile(
            "(UNDEF-(last|this|next)-(january|february|march|april|may|june|july|august|september|october|november|december))(.*)"
    );
    public static final Pattern PATTERN_REST_2D = Pattern.compile("-([0-9][0-9])");
    public static final Pattern PATTERN_SEASON_NAMES = Pattern.compile("(UNDEF-(last|this|next)-(SP|SU|FA|WI)).*");
    public static final Pattern PATTERN_PTdH = Pattern.compile("PT([0-9]+)H");
    public static final Pattern PATTER_PTdM = Pattern.compile("PT([0-9]+)M");
    public static final Pattern PATTERN_PdM = Pattern.compile("P([0-9]+)M");
    // TOOL NAME (may be used as componentId)
    private final Class<?> component = this.getClass();

    // PROCESSOR MANAGER
    private final ProcessorManager procMan = new ProcessorManager();
    ...
    public void initialize(UimaContext aContext) throws ResourceInitializationException {
        ...
        /////////////////////////////////
        Logger.setPrintDetails(doDebug);

        /////////////////////////////////
        // HANDLE LOCALE    		   //
        /////////////////////////////////
        this.language = Language.getLanguageFromString(languageString);

        if (requestedLocale == null || requestedLocale.isEmpty()) { // if the PARAM_LOCALE setting was left empty,
        ...
        chineseNumerals = new RegexHashMap<String>();
    ...
    public void process(JCas jcas) {
        ...
        ////////////////////////////////////////////
        FSIterator<Annotation> sentIter = jcas.getAnnotationIndex(Sentence.type).iterator();
        /*
                ...
                try {
                    if (find_dates) {
                        findTimexes(
                                ...
                                rulem.getHmDateFastCheck(),
                                rulem.getHmDatePosConstraint(),
                                rulem.getHmDateQuant(),
                                rulem.getHmDateFreq(),
                                rulem.getHmDateMod(),
                                rulem.getHmDateEmptyValue(),
                                s,
                                jcas
                        );
                    }
                    if (find_times) {
                        findTimexes(
                                ...
                                rulem.getHmTimeFastCheck(),
                                rulem.getHmTimePosConstraint(),
                                rulem.getHmTimeQuant(),
                                rulem.getHmTimeFreq(),
                                rulem.getHmTimeMod(),
                                rulem.getHmTimeEmptyValue(),
                                s,
                                jcas
                        );
                    }
                    ...
                    if (typeToProcess.equals("narrative") || typeToProcess.equals("narratives")) {
                        for (org.apache.uima.jcas.tcas.Annotation annotation : jcas.getAnnotationIndex(Timex3.type)) {
                            Timex3 t = (Timex3) annotation;
                    ...
                    if (find_sets) {
                        findTimexes(
                                ...
                                rulem.getHmSetFastCheck(),
                                rulem.getHmSetPosConstraint(),
                                rulem.getHmSetQuant(),
                                rulem.getHmSetFreq(),
                                rulem.getHmSetMod(),
                                rulem.getHmSetEmptyValue(),
                                s,
                                jcas
                        );
                    }
                    if (find_durations) {
                        findTimexes(
                                ...
                                rulem.getHmDurationFastCheck(),
                                rulem.getHmDurationPosConstraint(),
                                rulem.getHmDurationQuant(),
                                rulem.getHmDurationFreq(),
                                rulem.getHmDurationMod(),
                                rulem.getHmDurationEmptyValue(),
                                s,
                                jcas
                        );
                    }
                    if (find_temponyms) {
                        findTimexes(
                                ...
                                rulem.getHmTemponymFastCheck(),
                                rulem.getHmTemponymPosConstraint(),
                                rulem.getHmTemponymQuant(),
                                rulem.getHmTemponymFreq(),
                                rulem.getHmTemponymMod(),
                                rulem.getHmTemponymEmptyValue(),
                                s,
                                jcas
                        );
                    }
                    ...
                    if (!debugIteration) {
                        ...
                        Logger.printError(component, npe.fillInStackTrace().getMessage());
                        Logger.printError(
                                component,
        ...
        if (flagHistoricDates) try {
            ...
            Logger.printError(component, "Something went wrong disambiguating historic dates:\n" + e.fillInStackTrace().getMessage());
        }
...
//	public void findTimexesAlt(
//			final String timexType,
//			final HashMap<Pattern, String> hmPattern,
//			final HashMap<String, String> hmOffset,
//			final HashMap<String, String> hmNormalization,
//			final Sentence s,
//			final JCas jcas
//	) {
//		final RuleManager rm = RuleManager.getInstance(language, find_temponyms);
//
...
//
//		Map<Pattern, ArrayList<TimexResult>> results = hmPattern.entrySet().parallelStream().map(
//				entry -> new ImmutablePair<>(
//						entry.getKey(),
//						patternCheck(timexType, hmOffset, hmNormalization, rm, s, jcas, entry.getKey(), entry.getValue())
//				)
//		).filter(pair -> !pair.getRight().isEmpty()).collect(
//				Collectors.toMap(
//						Pair::getLeft,
//						Pair::getRight,
//						(k, v) -> k
...
//				continue;
//
//			for (TimexResult result : results.get(p)) {
//				addTimexAnnotation(
//						timexType,
//						result.timexStart + s.getBegin(),
//						result.timexEnd + s.getBegin(),
//						s,
//						result.attributes.value,
//						result.attributes.quant,
//						result.attributes.freq,
//						result.attributes.mod,
//						result.attributes.emptyValue,
//						"t" + timexID++,
//						hmPattern.get(p),
//						jcas
//				);
...
//	}
//
//	private record TimexResult(TimexAttributes attributes, int timexStart, int timexEnd) {
//	}
//
//	private ArrayList<TimexResult> patternCheck(
//			final String timexType,
//			final HashMap<String, String> hmOffset,
//			final HashMap<String, String> hmNormalization,
//			final RuleManager rm,
//			final Sentence s,
//			final JCas jcas,
...
//		Pattern fastPattern = switch (timexType) {
//			case "DATE" -> rm.getHmDateFastCheck().get(ruleName);
//			case "TIME" -> rm.getHmTimeFastCheck().get(ruleName);
//			case "DURATION" -> rm.getHmDurationFastCheck().get(ruleName);
//			case "SET" -> rm.getHmSetFastCheck().get(ruleName);
//			case "TEMPONYM" -> rm.getHmTemponymFastCheck().get(ruleName);
//			default -> null;
//		};
//
//		ArrayList<TimexResult> results = new ArrayList<>();
//		if (fastPattern == null || !fastPattern.matcher(s.getCoveredText()).find()) {
//			return results;
//		}
//
//		for (MatchResult r : Toolbox.findMatches(pattern, s.getCoveredText())) {
...
//			switch (timexType) {
//				case "DATE" -> {
//					if (hmDatePosConstraint.containsKey(ruleName)) {
//						posConstraintOK = checkPosConstraint(s, hmDatePosConstraint.get(ruleName), r, jcas);
//					}
//				}
//				case "DURATION" -> {
//					if (hmDurationPosConstraint.containsKey(ruleName)) {
//						posConstraintOK = checkPosConstraint(s, hmDurationPosConstraint.get(ruleName), r, jcas);
//					}
//				}
//				case "TIME" -> {
//					if (hmTimePosConstraint.containsKey(ruleName)) {
//						posConstraintOK = checkPosConstraint(s, hmTimePosConstraint.get(ruleName), r, jcas);
//					}
//				}
//				case "SET" -> {
//					if (hmSetPosConstraint.containsKey(ruleName)) {
//						posConstraintOK = checkPosConstraint(s, hmSetPosConstraint.get(ruleName), r, jcas);
//					}
//				}
//				case "TEMPONYM" -> {
//					if (hmTemponymPosConstraint.containsKey(ruleName)) {
//						posConstraintOK = checkPosConstraint(s, hmSetPosConstraint.get(ruleName), r, jcas);
//					}
//				}
//			}
...
//					TimexAttributes attributes = switch (timexType) {
//						case "DATE" -> getAttributesForTimexFromFile(
//								ruleName,
...
//								r
//						);
//						case "DURATION" -> getAttributesForTimexFromFile(
//								ruleName,
...
//								r
//						);
//						case "TIME" -> getAttributesForTimexFromFile(
//								ruleName,
...
//								r
//						);
//						case "SET" -> getAttributesForTimexFromFile(
//								ruleName,
...
//								r
//						);
//						case "TEMPONYM" -> getAttributesForTimexFromFile(
//								ruleName,
...
//
//					if (attributes != null) {
//						results.add(new TimexResult(attributes, timexStart, timexEnd));
//					}
//				} else {
//					Logger.printError("SOMETHING REALLY WRONG HERE: " + ruleName);
//				}
//			}
//		}
//		return results;
//	}
    ...
    public void addTimexAnnotation(
        ...
        FSIterator<Annotation> iterToken = jcas.getAnnotationIndex(Token.type).subiterator(sentence);
    ...
    public String specifyAmbiguousValuesString(
        ...
        // ////////////////////////////////////////////
        FSIterator<Annotation> dctIter = jcas.getAnnotationIndex(Dct.type).iterator();
        if (dctIter.hasNext()) {
            dctAvailable = true;
        ...
        ////////////////////////////////////////////////////
            ...
            if (ambigString.matches("^UNDEF-REFDATE$")) {
                if (i > 0) {
                    ...
                    valueNew = anyDate.getTimexValue();
                } else {
                    valueNew = "XXXX-XX-XX";
                }
                //////////////////
                ...
                //////////////////
                // year to calculate
            } else if (PATTERN_TO_CALCULATE.matcher(ambigString.trim()).matches()) {
                for (MatchResult mr : Toolbox.findMatches(PATTERN_TO_CALCULATE, ambigString)) {
                    ...
                    if ((documentTypeScientific)) {
                        ...
                        if (unit.equals("century")) {
                            ...
                            boolean checkForThis = (documentTypeNews || documentTypeColloquial || documentTypeScientific) && (dctAvailable) && (ltn.equals("this"));
                            switch (unit) {
                                case "decade" -> {
                                    if (checkForThis) {
            ...
            // MONTH NAMES
            else if (PATTERN_MONTH_NAMES.matcher(ambigString.trim()).matches()) {
                for (MatchResult mr : Toolbox.findMatches(PATTERN_MONTH_NAMES, ambigString)) {
                    String rest = mr.group(4);
                    int day = 0;
                    for (MatchResult mr_rest : Toolbox.findMatches(PATTERN_REST_2D, rest)) {
            ...
            // SEASONS NAMES
            else if (PATTERN_SEASON_NAMES.matcher(ambigString.trim()).matches()) {
                for (MatchResult mr : Toolbox.findMatches(PATTERN_SEASON_NAMES, ambigString)) {
                    String checkUndef = mr.group(1);
                    String ltn = mr.group(2);
                    ...
                    switch (ltn) {
                        case "last" -> {
                            if ((documentTypeNews || documentTypeColloquial || documentTypeScientific) && (dctAvailable)) {
                                switch (dctSeason) {
                                    case "SP" -> valueNew = valueNew.replace(checkUndef, dctYear - 1 + "-" + newSeason);
                                    case "SU" -> {
                                ...
                                if (lmSeason.isEmpty()) {
                                    ...
                                    final String lastYearNewSeason = Integer.parseInt(lmSeason.substring(0, 4)) - 1 + "-" + newSeason;
                                    final String thisYearNewSeason = Integer.parseInt(lmSeason.substring(0, 4)) + "-" + newSeason;
                                    switch (lmSeason.substring(5, 7)) {
                                        case "SP" -> valueNew = valueNew.replace(checkUndef, lastYearNewSeason);
                                        case "SU" -> {
                                            if (newSeason.equals("SP")) {
                                                valueNew = valueNew.replace(checkUndef, thisYearNewSeason);
                                            } else {
                                                valueNew = valueNew.replace(checkUndef, lastYearNewSeason);
                                            }
                                        }
                                        case "FA" -> {
                                            if ((newSeason.equals("SP")) || (newSeason.equals("SU"))) {
                        ...
                        case "next" -> {
                            if ((documentTypeNews || documentTypeColloquial || documentTypeScientific) && (dctAvailable)) {
                                switch (dctSeason) {
                                    ...
                                    case "WI" -> valueNew = valueNew.replace(checkUndef, dctYear + 1 + "-" + newSeason);
                                }
                                ...
                                if (lmSeason.isEmpty()) {
                                    ...
                                    final String nextYearNewSeason = Integer.parseInt(lmSeason.substring(0, 4)) + 1 + "-" + newSeason;
                                    final String thisYearNewSeason = Integer.parseInt(lmSeason.substring(0, 4)) + "-" + newSeason;
                                    switch (lmSeason.substring(5, 7)) {
                                        case "SP" -> {
                                            if (newSeason.equals("SP")) {
                                                valueNew = valueNew.replace(checkUndef, nextYearNewSeason);
                                            } else {
                                                valueNew = valueNew.replace(checkUndef, thisYearNewSeason);
                                            }
                                        }
                                        ...
                                        case "WI" -> valueNew = valueNew.replace(checkUndef, nextYearNewSeason);
                                    }
                                }
                            }
                        }
    ...
    public void specifyAmbiguousValues(JCas jcas) {
        ...
        for (Annotation annotation : jcas.getAnnotationIndex(Timex3.type)) {
            ...
            if (timex.getTimexType().equals("DURATION") && !timex.getEmptyValue().isEmpty()) {
                linearDates.add(timex);
            }
        }
        ...
        for (ArrayList<Timex3> tSet : effectivelyToInspect) {
            ...
            for (Timex3 t : tSet) {
                ...
                if (longestTimex == null) {
                    longestTimex = t;
                } else if (allSameTypes && t.getFoundByRule().contains("-BCADhint")) {
                    longestTimex = t;
                } else if (
                        allSameTypes &&
                                !t.getFoundByRule().contains("relative") &&
                                longestTimex.getFoundByRule().contains("relative")
                ) {
                    longestTimex = t;
            ...
            if (allSameTypes) {
                ...
                if (!tokenIds.isEmpty()) newTimex.setFirstTokId(tokenIds.getFirst());
                StringBuilder tokenIdText = new StringBuilder("BEGIN");
                for (Integer tokenId : tokenIds) {
                    tokenIdText.append("<-->").append(tokenId);
                }
                newTimex.setAllTokIds(tokenIdText.toString());
            }
     ...
     */
    public static String getPosFromMatchResult(int tokBegin, int tokEnd, Sentence s, JCas jcas) {
        // get all tokens in sentence
        HashMap<Integer, Token> hmTokens = new HashMap<Integer, Token>();
        FSIterator<Annotation> iterTok = jcas.getAnnotationIndex(Token.type).subiterator(s);
    ...
    public void findTimexes(
            ...
            HashMap<String, Pattern> hmFastCheck,
            HashMap<String, String> hmPosConstraint,
        ...
        for (Pattern p : Toolbox.sortByValue(hmPattern)) {
            ...
            Pattern fastCheckPattern = hmFastCheck.get(ruleName);
            if (fastCheckPattern != null && !fastCheckPattern.matcher(s.getCoveredText()).find()) {
                continue;
            }

            for (MatchResult matchResult : Toolbox.findMatches(p, s.getCoveredText())) {
                // improved token boundary checking
                boolean infrontBehindOK = ContextAnalyzer.checkTokenBoundaries(matchResult, s, jcas) &&
                        ContextAnalyzer.checkInfrontBehind(matchResult, s);

                // CHECK POS CONSTRAINTS
                boolean posConstraintOK = !hmPosConstraint.containsKey(ruleName) ||
                        checkPosConstraint(s, hmPosConstraint.get(ruleName), matchResult, jcas);

                if ((infrontBehindOK) && (posConstraintOK)) {
                    // Offset of timex expression (in the checked sentence)
                    int timexStart = matchResult.start();
                    int timexEnd = matchResult.end();
                    ...
                    if (hmOffset.containsKey(ruleName)) {
                        ...
                        // pattern for offset information
                        for (MatchResult mr : Toolbox.findMatches(PATTERN_OFFSET, offset)) {
                            int startOffset = Integer.parseInt(mr.group(1));
                            int endOffset = Integer.parseInt(mr.group(2));
                            timexStart = matchResult.start(startOffset);
                            timexEnd = matchResult.end(endOffset);
                        }
                    }

                    // Normalization Parameter
                    if (hmNormalization.containsKey(ruleName)) {
                        TimexAttributes attributes = getAttributesForTimexFromFile(
                                ruleName, hmNormalization, hmQuant, hmFreq, hmMod, hmEmptyValue, matchResult
                        );
     ...
     */
    public static boolean checkPosConstraint(Sentence s, String posConstraint, MatchResult m, JCas jcas) {
        for (MatchResult mr : Toolbox.findMatches(PATTERN_CONSTRAINT, posConstraint)) {
    ...
    public String applyRuleFunctions(String tonormalize, MatchResult m) {
        ...
        while ((tonormalize.contains("%")) || (tonormalize.contains("group"))) {
            ...
            for (MatchResult mr : Toolbox.findMatches(PATTERN_NORM, tonormalize)) {
                Logger.printDetail("-----------------------------------");
            ...
            for (MatchResult mr : Toolbox.findMatches(PATTERN_GROUP, tonormalize)) {
                Logger.printDetail("-----------------------------------");
                Logger.printDetail("DEBUGGING: tonormalize:" + tonormalize);
                Logger.printDetail("DEBUGGING: mr.group():" + mr.group());
                Logger.printDetail("DEBUGGING: mr.group(1):" + mr.group(1));
            ...
            // replace substrings
            for (MatchResult mr : Toolbox.findMatches(PATTERN_SUBSTRING, tonormalize)) {
            ...
            if (language.getName().compareTo("arabic") != 0) {
                // replace lowercase
                for (MatchResult mr : Toolbox.findMatches(PATTERN_LOWERCASE, tonormalize)) {
                    String substring = mr.group(1).toLowerCase();
                    tonormalize = tonormalize.replace(mr.group(), substring);
                }

                // replace uppercase
                for (MatchResult mr : Toolbox.findMatches(PATTERN_UPPERCASE, tonormalize)) {
                    String substring = mr.group(1).toUpperCase();
                    tonormalize = tonormalize.replace(mr.group(), substring);
                }
            }
            // replace sum, concatenation
            for (MatchResult mr : Toolbox.findMatches(PATTERN_SUM, tonormalize)) {
            ...
            for (MatchResult mr : Toolbox.findMatches(PATTERN_NORM_NO_GROUP, tonormalize)) {
                tonormalize = tonormalize.replace(
                        mr.group(),
            ...
            for (MatchResult mr : Toolbox.findMatches(PATTERN_CHINESE_NORM, tonormalize)) {
    ...
    public TimexAttributes getAttributesForTimexFromFile(
            String rule,
            HashMap<String, String> hmNormalization,
            HashMap<String, String> hmQuant,
            HashMap<String, String> hmFreq,
        ...
        String value = applyRuleFunctions(value_normalization_pattern, m);
        ...
        return new TimexAttributes(value, quant, freq, mod, emptyValue);
    }
    ...
    public String correctDurationValue(String value) {
        try {
            Matcher matcher = PATTERN_PTdH.matcher(value);
            if (matcher.matches()) {
                int hours = Integer.parseInt(matcher.toMatchResult().group(1));
                if ((hours % 24) == 0) {
                    int days = hours / 24;
                    return value = "P" + days + "D";
                }
            }
            matcher = PATTER_PTdM.matcher(value);
            if (matcher.matches()) {
                int minutes = Integer.parseInt(matcher.toMatchResult().group(1));
                if ((minutes % 60) == 0) {
                    int hours = minutes / 60;
                    return "PT" + hours + "H";
                }
            }
            matcher = PATTERN_PdM.matcher(value);
            if (matcher.matches()) {
                int months = Integer.parseInt(matcher.toMatchResult().group(1));
                if ((months % 12) == 0) {
                    int years = months / 12;
                    return "P" + years + "Y";
                }
