 ...
 *
 * @author jannik stroetgen, Andy Lücking, Manuel Schaaf
 */
abstract public class HeidelTime extends JCasAnnotator_ImplBase {
    ...
    public static final Pattern PATTERN_REST_D2 = Pattern.compile("-([0-9][0-9])");
    ...
    public static final Pattern PATTERN_PTdM = Pattern.compile("PT([0-9]+)M");
    public static final Pattern PATTERN_PdM = Pattern.compile("P([0-9]+)M");
    /// Something like {@code 20041224}
    public static final Pattern PATTERN_DCT_D8 = Pattern.compile("\\d{8}");
    /// Something like {@code 2004-12-24}
    public static final Pattern PATTERN_DCT_D422 = Pattern.compile("\\d{4}.\\d{2}.\\d{2}.*");

    // PROCESSOR MANAGER
    public final ProcessorManager procMan = new ProcessorManager();
    ...
    protected String typeToProcess;
    ...
    protected boolean deleteOverlapped;
...
//    protected boolean parallelSearch;
    ...
    protected RegexHashMap<String> chineseNumerals;
    ...
    public void initialize(UimaContext aContext) throws ResourceInitializationException {
        ...
        /////////////////////////////////
        getLogger().setLevel(doDebug ? Level.DEBUG : Level.INFO);
        ...
        if (requestedLocale == null || requestedLocale.isEmpty()) { // if the PARAM_LOCALE setting was left empty,
            ...
            try {
                ...
                getLogger().info("Supplied locale parameter couldn't be resolved to a working locale. Try one of these:" + localesString);
                System.exit(-1);
            }
        }
        ...
        ///////////////////////////////////////////////////
        RuleManager.getRuleSet(Language.getLanguageFromString("english"), find_temponyms);
        RuleManager.getRuleSet(Language.getLanguageFromString("german"), find_temponyms);
        ...
        // Initialize Chinese Number Lookup
        chineseNumerals = new RegexHashMap<>();
        ...
        /////////////////////////////
        if (find_dates) getLogger().info(
                "HeidelTime Config: find_dates={}, find_times={}, find_durations={}, find_sets={}, find_temponyms={}",
                find_dates, find_times, find_durations, find_sets, find_temponyms
        );
    }
    ...
    public String specifyAmbiguousValuesString(
        ...
        ////////////////////////////////////////////////////
            ...
            if (ambigString.matches("^UNDEF-REFDATE$")) {
                ...
                for (MatchResult mr : Utils.findMatches(PATTERN_TO_CALCULATE, ambigString)) {
                    ...
                    try {
                        ...
                        getLogger().error("""
                                        Expression difficult to normalize: {}
                                        {} probably too long for parsing as integer.
                                        sets normalized value as PAST_REF / FUTURE_REF: {}""",
                                ambigString,
                                sDiff,
                                op.equals("PLUS") ? "FUTURE_REF" : "PAST_REF"
                        );
                        break;
                    }
            ...
            // MONTH NAMES
                ...
                for (MatchResult mr : Utils.findMatches(PATTERN_MONTH_NAMES, ambigString)) {
                    String rest = mr.group(4);
                    int day = 0;
                    for (MatchResult mr_rest : Utils.findMatches(PATTERN_REST_D2, rest)) {
            ...
            // SEASONS NAMES
                ...
                for (MatchResult mr : Utils.findMatches(PATTERN_SEASON_NAMES, ambigString)) {
                    String checkUndef = mr.group(1);
                    String ltn = mr.group(2);
            ...
            // TODO tense should be included?!
                ...
                for (MatchResult mr : Utils.findMatches(
                        Pattern.compile(
                                "(UNDEF-(last|this|next|day)-(monday|tuesday|wednesday|thursday|friday|saturday|sunday)).*"
                        ),
                        ambigString
                ...
                getLogger().debug("ATTENTION: UNDEF value for: {} is not handled in disambiguation phase!", valueNew);
            }
        }
    ...
    /**
     * Under-specified values are disambiguated here. Only Timexes of types "dates" and "times" can be under-specified.
     */
    ...
    public void deleteOverlappedTimexesPostprocessing(JCas jcas) {
        ...
        for (Set<Timex3> tSet : effectivelyToInspect) {
            ...
            /*
             * check
             * - whether all timexes of this sets have the same timex type attribute,
             * - which one in the sets has the longest value attribute string length,
            ...
            for (Timex3 t : tSet) {
                ...
                getLogger().debug("Are these overlapping timexes of same type? => " + allSameTypes);
                ...
                getLogger().debug("Selected combined constraints: " + combinedBegin + ":" + combinedEnd);
...
//                }
    ...
    public static Set<Timex3> getIntersection(Timex3 pivot, TreeMap<Integer, Set<Timex3>> beginTreeMap, TreeMap<Integer, Set<Timex3>> endTreeMap) {
    ...
    public String correctDurationValue(String value) {
        try {
            ...
            matcher = PATTERN_PTdM.matcher(value);
            if (matcher.matches()) {
            ...
            getLogger().error("Couldn't do granularity conversion for " + value);
        }
     ...
     */
    public boolean isValidDCT(JCas jcas) {
        FSIterator<Annotation> dctIter = jcas.getAnnotationIndex(Dct.type).iterator();
